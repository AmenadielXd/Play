...
from Play.misc import SUDOERS, db
...
from Play.utils.formatters import seconds_to_min
from Play.utils.inline import close_markup, stream_markup, stream_markup_timer
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            run = await CallbackQuery.message.reply_photo(
                ...
                caption=_["stream_2"].format(f"<a href='https://t.me/{BOT_USERNAME}'>{BOT_NAME}</a>", user),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "tg"
            ...
            if videoid == "telegram":
                ...
                run = await CallbackQuery.message.reply_photo(
                    ...
                    caption=_["stream_1"].format(f"<a href='https://t.me/{BOT_USERNAME}'>{BOT_NAME}</a>", SUPPORT_CHAT, title[:23], duration, user
                    ),
                    reply_markup=InlineKeyboardMarkup(button),
                )
                db[chat_id][0]["mystic"] = run

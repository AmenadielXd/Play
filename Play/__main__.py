...
import config
from Play import LOGGER, app, userbot
from Play.core.call import Anony
from Play.misc import sudo
from Play.plugins import ALL_MODULES
from Play.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("Play.plugins" + all_module)
    LOGGER("Play.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Anony.start()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("Play").info("Stopping Alex Music Bot...")
